...
ocr = PaddleOCR(
    show_log=False, use_angle_cls=True, use_gpu=False, lang="en"
)  # Creating a PaddleOCR object for text recognition
...
class ImageHandler:
    ...
    def open_image(img_path):
        ...
        if img_path:
            ...
            )  # преобразуем изображение в масив Numpy
...
class CarPlateExtractor:
    ...
    def extract(self, image):
        ...
        carplate_rects = self.classifier.detectMultiScale(
            image, scaleFactor=1.04, minNeighbors=6, minSize=(30, 30)
        )
        carplate_imgs = [
            image[y + 10 : y + h - 5, x + 10 : x + w - 5]
...
class OCRProcessor:
    ...
    def process(self, image):
        ...
        result = self.ocr.ocr(image, cls=True, det=True, rec=True)
        return result
            ...
            for carplate_img in carplate_imgs:
                ...
                if result and result[0]:
                    for line in result[0]:
                        print(
                            f"Recognized text: {line[1][0]} (Confidence: {line[1][1]:.2f})"
                        )
...
if __name__ == "__main__":
    recognizer = CarPlateRecognizer(
        classifier_path="model/haarcascade_russian_plate_number.xml",
